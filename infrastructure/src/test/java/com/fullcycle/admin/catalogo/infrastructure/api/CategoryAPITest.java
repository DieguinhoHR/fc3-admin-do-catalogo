...
class CategoryAPITest {
    ...
    // Nos ajuda a fazer chamadas HTTP simuladas
    @Autowired
    private MockMvc mvc;
    ...
    // Crie um versão deste bean totalmente mockada
    @MockBean
    ...
    @Test
    void givenACommandWithInvalidID_whenCallsUpdateCategory_shouldReturnNotFoundDomainException() throws Exception {
        // given
    ...
    @Test
    void givenAInvalidName_whenCallsCreateCategory_thenShouldReturnDomainException() throws Exception {
        // given
        final var expectedId = "123";
        final var expectedName = "Filmes";
        final var expectedDescription = "A categoria mais assistida";
        ...
        final var expecteMessage = "'name' should not be null";
        final var expectedErrorCount = 1;

        when(updateCategoryUseCase.execute(any()))
                .thenReturn(Left(Notification.create(new Error(expecteMessage))));
        ...
        response.andExpect(status().isUnprocessableEntity())
                ...
                .andExpect(jsonPath("$.errors[0].message", equalTo(expecteMessage)));
